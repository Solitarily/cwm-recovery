...
toggle_signature_check()
{
    ...
    ui_print("签名检测：%s\n", signature_check_enabled ? "已打开" : "已关闭");
}
...
{
    static char* headers[] = {  "安装zip刷机包",
                                "",
                                NULL
    };
    
    char* install_menu_items[] = {  "从SD卡选择安装包",
                                    "从adb sideload读取安装包",
                                    "从/sdcard/update.zip读取安装包",
                                    "安装包签名检测开关",
                                    NULL,
                                    NULL };
    ...
    for (;;)
    {
        ...
        switch (chosen_item)
        {
            ...
            {
                if (confirm_selection("确定安装？", "是 - 安装/sdcard/update.zip"))
...
void show_choose_zip_menu(const char *mount_point)
{
    ...
    if (file == NULL)
        return;
    static char* confirm_install  = "确定要安装？";
...
void show_nandroid_restore_menu(const char* path)
{
    ...
    if (confirm_selection("确定要还原？", "是 - 还原"))
...
void show_nandroid_delete_menu(const char* path)
{
    ...
    if (confirm_selection("确定要删除？", "是 - 删除")) {
        // nandroid_restore(file, 1, 1, 1, 1, 1, 0);
...
void show_mount_usb_storage_menu()
{
    ...
    static char* headers[] = {  "USB大容量存储设备",
                                "离开这个页面退出U盘模式",
                                "你的电脑将不能访问SD卡",
                                "",
                                NULL
...
int format_unknown_device(const char *device, const char* path, const char *fs_type)
{
    ...
    if (0 == strcmp(path, "/sd-ext"))
    {
        ...
        {
            ui_print("未找到app2sd分区，跳过格式化/sd-ext\n");
            return 0;
        }
    }
...
void show_partition_menu()
{
    ...
    static char* confirm_format  = "确定要格式化？";
    ...
    for (;;)
    {
        ...
        if (!is_data_media()) {
          options[mountable_volumes + formatable_volumes] = "打开U盘模式";
          options[mountable_volumes + formatable_volumes + 1] = NULL;
        }
        else {
          options[mountable_volumes + formatable_volumes] = "格式化data和内置存储";
          options[mountable_volumes + formatable_volumes + 1] = NULL;
        }
        ...
        if (chosen_item == (mountable_volumes+formatable_volumes)) {
            if (!is_data_media()) {
                ...
                if (!confirm_selection("格式化data和内置存储", confirm))
                    continue;
...
void show_nandroid_advanced_restore_menu(const char* path)
{
    ...
    static char* advancedheaders[] = {  "选择一个备份",
                                "",
                                "选择要还原的备份",
                                "先选择一个备份",
                                "下一步有更多选项",
    ...
    static char* confirm_restore  = "确定要还原？";

    int chosen_item = get_menu_selection(headers, list, 0, 0);
    switch (chosen_item)
    {
        case 0:
            if (confirm_selection(confirm_restore, "是 - 还原boot"))
                nandroid_restore(file, 1, 0, 0, 0, 0, 0);
            break;
        case 1:
            if (confirm_selection(confirm_restore, "是 - 还原system"))
                nandroid_restore(file, 0, 1, 0, 0, 0, 0);
            break;
        case 2:
            if (confirm_selection(confirm_restore, "是 - 还原data"))
                nandroid_restore(file, 0, 0, 1, 0, 0, 0);
            break;
        case 3:
            if (confirm_selection(confirm_restore, "是 - 还原cache"))
                nandroid_restore(file, 0, 0, 0, 1, 0, 0);
            break;
        case 4:
            if (confirm_selection(confirm_restore, "是 - 还原sd-ext"))
                nandroid_restore(file, 0, 0, 0, 0, 1, 0);
            break;
        case 5:
            if (confirm_selection(confirm_restore, "是 - 还原wimax"))
...
static void partition_sdcard(const char* volume) {
    ...
    static char* ext_headers[] = { "分区大小", "", NULL };
    static char* swap_headers[] = { "Swap大小", "", NULL };
    static char* fstype_headers[] = {"分区类型", "", NULL };
    ...
    ui_print("正在对SD卡分区...请等待...\n");
    if (0 == __system(cmd))
        ui_print("完成\n");
    else
        ui_print("进行SD卡分区时发生一个未知错误，查看/tmp/recovery.log获取详细信息\n");
}
...
{
    static char* headers[] = {  "高级选项",
                                "",
                                NULL
    };

    static char* list[] = { "重启到恢复模式",
                            "重启到引导模式",
                            "关机",
                            "清空Dalvik Cache",
                            "复制错误日志",
    ...
    for (;;)
    {
        ...
        switch (chosen_item)
        {
            ...
            case 3:
                ...
                if (confirm_selection( "确定清除？", "是 - 清除Dalvik Cache")) {
...
void handle_failure(int ret)
{
    ...
    ui_print("/tmp/recovery.log 已复制到 /sdcard/clockworkmod/recovery.log\n");
}
    ...
    if (0 != lstat("/system/etc/.installed_su_daemon", &st)) {
        ...
        if (0 == lstat("/system/etc/install-recovery.sh", &st)) {
            if (st.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("固件可能会恢复原版Recovery，是否修复？", "是 - 禁用install-recovery.sh脚本")) {
    ...
    if (0 == lstat("/system/bin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            if ((st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("修复ROOT权限？", "是 - 修复ROOT (/system/bin/su)")) {
    ...
    if (0 == lstat("/system/xbin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            if ((st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("修复ROOT权限？", "是 - 修复ROOT (/system/xbin/su)")) {
