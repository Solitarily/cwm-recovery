...
int nandroid_backup(const char* backup_path)
{
    ...
    if (NULL != volume) {
        ...
        ui_print("SD剩余空间：%lluMB\n", sdcard_free_mb);
        if (sdcard_free_mb < 150)
            ui_print("剩余空间可能不足...仍然继续...\n");
    }
    char tmp[PATH_MAX];
    ...
    if (vol != NULL && 0 == stat(vol->device, &s))
    {
        ...
        if (0 != ret)
            return print_and_error("创建WiMAX分区镜像时发生错误\n");
    }
    ...
    if (is_data_media() || 0 != stat("/sdcard/.android_secure", &s)) {
        ui_print("未找到.android_secure 跳过备份外置卡上的应用程序\n");
    }
    else {
